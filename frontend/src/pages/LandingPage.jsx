...
const LandingPage = () => {
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-800 flex flex-col relative overflow-hidden">
      ...
      <div className="absolute top-0 left-1/2 w-[1000px] h-[600px] bg-indigo-600/5 rounded-full blur-3xl -translate-x-1/2 -translate-y-1/2 pointer-events-none"></div>
      ...
      <section className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-20 pb-16 text-center">
        <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-indigo-50 border border-indigo-100 text-indigo-600 text-xs font-semibold mb-6">
          <span>🌟 Premium Showroom & Inventory Manager</span>
        </div>
        ...
        <div className="flex flex-col sm:flex-row justify-center items-center gap-4">
          {user ? (
            ...
            <>
              ...
              <Link
                to="/register"
                className="w-full sm:w-auto bg-white hover:bg-slate-100 text-slate-700 font-semibold px-8 py-4 rounded-xl border border-slate-200 shadow-sm transition duration-200"
              >
                Create an Account
              </Link>
            </>
      ...
      <section className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6 bg-white border border-slate-200/80 rounded-2xl p-8 shadow-sm">
          {stats.map((stat, idx) => (
            <div key={idx} className="text-center">
              <p className="text-3xl md:text-4xl font-black bg-gradient-to-r from-indigo-600 to-purple-600 bg-clip-text text-transparent mb-1">
                {stat.value}
              </p>
      ...
      <section className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {features.map((feature, idx) => (
            <div
              key={idx}
              className="bg-white border border-slate-200/80 rounded-2xl p-8 hover:shadow-md transition duration-300 flex flex-col gap-4 shadow-sm"
            >
      ...
      {/* Footer */}
      <footer className="mt-auto border-t border-slate-200/80 bg-white py-8 text-center text-slate-400 text-sm z-10">
